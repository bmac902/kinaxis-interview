...
import { costByProject } from '../data/mockData'
...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-slate-800 border border-slate-600 rounded-lg p-3 text-xs shadow-xl">
      ...
      <p className="text-slate-400 mt-1">3-Month Total: <span className="text-white font-mono font-semibold">{fmt(d.cost)}</span></p>
      {!d.tagged && (
        <p className="text-red-400 mt-1 font-medium">⚠ Untagged — no team attribution</p>
      )}
    </div>
  )
}
...
export default function CostByProject() {
  const data = [...costByProject].sort((a, b) => b.cost - a.cost)
  ...
  return (
    <div className="bg-slate-900 border border-slate-700/50 rounded-xl p-5 h-full">
      <div className="mb-4">
        ...
        <p className="text-xs text-slate-500 mt-0.5">3-month total · SubAccountId · BilledCost</p>
      </div>
        ...
        >
          ...
          <YAxis
            ...
            tick={({ x, y, payload }) => {
              ...
              const isUntagged = !data.find(d => d.project === payload.value)?.tagged
              return (
